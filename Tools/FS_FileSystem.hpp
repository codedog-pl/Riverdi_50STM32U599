...
public:
    ...
    {
        for (int i = 0; i < max; i++) if (entries[i].media == media) return &entries[i];
        return nullptr;
    }
...
private:
    ...
    {
        for (int i = 0; i < max; ++i) if (!entries[i].root) return &entries[i];
        return nullptr;
    }
    ...
    {
        if (!entry) return;
        for (int i = 0; i < max; ++i) if (&entries[i] == entry) { entry->clear(); return; }
    }
